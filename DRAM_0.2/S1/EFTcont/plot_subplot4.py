...
def main():
	...
	plt.ylabel('Mean Aks')
	plt.xlabel('Time (Day)')
	plt.title('Daily Average Aks in Continuous Model')
...
#	plt.xlabel('Time (Day)')
#	plt.title('Daily Surface Nutrient Limitation Index in Continuous Model')
#       month_vec = np.tile(['N', 'D', 'J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D', 'J'], 1)
 #       plt.xticks(np.arange(0, 450, 30), month_vec)
#	plt.axis([0, len(plot_params_lno), 0.3, max(plot_params_lno)+0.1*(max(plot_params_lno))])
	...
	plt.xlabel('Time (Day)')
	plt.title('Daily Surface $NPP_T$ in Continuous Model')
        month_vec = np.tile(['N', 'D', 'J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D', 'J'], 1)
        plt.xticks(np.arange(0, 450, 30), month_vec)
