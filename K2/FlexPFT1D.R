...
lon      <- 160
lat      <- 47
dt       <- 60  #Time step, unit: s
depth    <- 250 #Total depth (m) of the station
nlev     <- 20  #Total number of depth layers
MaxN     <- 360 #Maximal number of days to run
...
for (var in c('temp','par','Aks','NO3')){
    ...
    outfile  <-  paste(Stn_name,'_',var,'.dat',sep='')
...
if (length( dim(data) ) == 2) {
  ...
  image2D(data1, x=month, y=rev(depth),
              xlab="Month", ylab="Depth (m)")
  axis(1, at = seq(0.5,11.5,by=1))
  axis(2, at = seq(0,min(depth),by=-30)) 
