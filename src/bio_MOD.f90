...
integer, public  :: imu0,iaI0,iV0N,igmax,iKN, ithetm, iPHYini
...
SELECT CASE(model_ID)
  case(Geidersimple)
   ...
   CASE(EFTSIMPLE)
    ...
    oD_DET=oD_ZOO+1
    Nout  =oD_DET
    write(6,'(A8, I3, A25)') 'Totally ',Nout,'Variables for diagosis!'
    allocate(Varout(  Nout,nlev))
    allocate(Labelout(Nout+ ow ))
...
implicit none
integer :: k
real    :: rhochl, theta, NO3, PHY, ZOO, CHL, DET
real    :: rmax_T, SI, muNet, tf_z, INGES, EGES
real    :: RES, Zmort, DET1
...
DO k = 1, nlev
   ...
   SI = 1d0 - exp(-params(iaI0) * dtdays *PAR(k) * theta/rmax_T)
   ...
   rhochl = thetm*muNet/(params(iaI0)*dtdays*PAR(k)*theta)
   ...
   INGES  = ZOO*params(igmax)*tf_z * dtdays            &
            * PHY**2/(  params(ikp)**2 + PHY**2 )
...
real    :: W_Y,ThetaHat,SI,mu0hatSI,muIhat
real    :: Zmort,RES,EGES,INGES,Lno3
real    :: pp_ND,pp_NZ,pp_PN,pp_DZ,pp_ZP
real    :: PHY,NO3,ZOO,DET
...
! Calculate thetahat (optimal g Chl/mol C for the chloroplast under nutrient replete conditions)
! Only calculate within the euphotic zone, otherwise many numerical problems.
  if( PAR(k) .gt. I_zero ) then
    larg1 = exp(1d0 + aI*PAR(k)/(mu0hat*zetaChl))
    larg  = (1d0 + RMchl/mu0hat)*larg1   
    
     W_Y  = WAPR(larg,0,0)

   ThetaHat = 1d0/zetaChl + (1d0- W_Y)*mu0hat/(aI * PAR(k))

    ! Effect of light limitation
    SI = 1d0 - max(exp(-aI*PAR(k)*ThetaHat/mu0hat),0d0)
    Varout(oSI(1),k) = SI
! Light dependent growth rate 
! (needs to take into account the cost of dark and light-dependent chl maintenance)
    mu0hatSI = mu0hat*SI !Gross specific carbon uptake (photosynthesis)
...
!subroutine Phygrowth_size(PMU, NO3, tC, par_, muNet, QN, Theta, wPHY,  LSI, Lno3)
     call Phygrowth_size(PMU_(i),Vars(iNO3,k),Temp(k),PAR(k), &
     ...
     do j = 1, 4
        ...
        !if (CHLs(j) .lt. 0d0) then
        !   write(6,*) 'PHY =',Vars(iPHY(i),k)
        !   write(6,*) 'QN =',Varout(oQN(i),k)
        !   write(6,*) 'Theta =',Varout(oTheta(i),k)
        !   write(6,*) 'j = ',j
        !   write(6,*) 'wtCHL(j,i) = ',wtCHL(j,i)
        !   stop
        !endif
   ...
   do j = 1,4
      ...
      if (CHLs(j) .lt. 0d0) then
           !write(6,*) 'PHY =',Vars(iPHY(i),k)
           !write(6,*) 'QN =',Vars(oQN(i),k)
           !write(6,*) 'Theta =',Vars(oTheta(i),k)
           write(6,*) 'j = ',j
           write(6,*) 'CHLs = ',CHLs(j)
           stop
        endif
