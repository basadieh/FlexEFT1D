...
real, private, parameter :: dtsec  = 12d2     ! time step in seconds
...
integer,          private, parameter :: NFobs(TNFo)     = (/    12,   12,   12,     12,  12 /)
...
real, private :: obs_Aks( N_Aks ,  1+NFobs(eAks ))
...
! Assign the data dimension, must be consistent with external file:
  ...
  allocate( TINData(nrow(1),ncol(1)))
  allocate( CHLData(nrow(2),ncol(2)))
  allocate( NPPData(nrow(3),ncol(3)))
  allocate( OBSData(TNobs  ,3      ))
  ...
  if (Model_ID == EFTdiscrete .or. Model_ID == EFTcont) then
     allocate(SizeData(nrow(4),ncol(4)))
     SizeData(:,:) = 0d0
...
do k = 1,nlev
   do i = 1,NPHY
      Vars(iPHY(i), k) = params(iPHYini)/float(NPHY)
   enddo
   do i = (iPHY(NPHY)+1), NVAR
      Vars(i,k) = 1D-2
   enddo
enddo
...
!subroutine time_interp(time, N, nrow, obs_time, obs_data, mod_data)
  ...
  IF (mod(it, nsave) .EQ. 1) THEN
     ...
     ! write(6,'(A3,I2,A3,1pe12.3)') 'w(',1,') =',w(1)
  ! Test if total nitrogen conserved:
    ! call Cal_TN
    ! write(6,*) 'Total nitrogen = ',Ntot
     !do k = 1, 2
     !   do i = 1, NVAR
     !      write(6,*) Labelout(i+ow),' at',Z_r(k),'m = ',Vars(i,k)
     !   enddo
     !enddo
