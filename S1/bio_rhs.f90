...
! Model parameters:
  ...
  w_p0   =0.,   alphaW=0.,   Ep   =0.4,Femin =0.02, K0Fe =0.8,       &
  alphaFe=0.14, alphaG=1.1,zetaChl=0.8, zetaN= 0.6,RMchl0=0.1,       &
  Wmax=50.0,K0N=0.2,alphaK=0.27,Ez=0.6, rdN=0.1,   mz    =0.05,      &
  GGE =0.3, unass=0.24
...
!========================================================
subroutine Phygrowth_size(PMU, NO3, tC, par_,muNet, QN, Theta, wPHY)
  implicit none
!INPUT PARAMETERS:
  real(8), intent(in)    :: PMU, NO3, tC, par_
  real(8), intent(out)   :: muNet, QN, Theta, wPHY
!LOCAL VARIABLES of phytoplankton:
  real(8) :: tf_p,I_zero,X
  real    :: larg   !Environmental variables
  real(8) :: KFe,Fe
  real(8) :: V0hat,Kn,Lmin,A0N,A0hat,fA
  real(8) :: VNhat
  real(8) :: mu0hat,muIhat,mu0hatSI,RMchl
  real(8) :: fV
  real(8) :: ZINT
  real(8) :: aI,SI
  real(8) :: ThetaHat
...
! Only calculate within the euphotic zone, otherwise many numerical problems.
  ...
  if( par_ .gt. I_zero ) then
    ...
    larg1 = exp(1. + min(aI*par_/(mu0hat*zetaChl),600.))

    larg  = (1. + RMchl/mu0hat)*larg1   
    
   W_Y      = WAPR(larg,0,0)
   ThetaHat = 1.0/zetaChl + (1.0- W_Y)*mu0hat/(aI * par_)
...
SUBROUTINE FLEXEFT_DISC
  implicit none
  integer            :: i,k
  real(8), parameter :: PMU_min=0.123, PMU_max=15.
  ...
  DO k=1,nlev
     ...
     PHYtot =0.  ! Calculate total PHY biomass
     PHYtot2=0.  ! total P**alphaG
     pp_PN  =0.  ! total primary production (mmol N per d per m3)
...
!! ZOOplankton section:
   ...
   !Zooplankton total ingestion rate
     INGES = tf_z*dtdays*params(iCmax)*PHYtot
...
!! Solve ODE functions:
   ...
   DET  = Vars(iDET,k)
   DET1 = (DET+pp_DZ)/(1. + dtdays*rDN*tf_z )

   Varout(oDET,k) = min(max(DET1,1D-9),1D5)
   
   NO3  = Vars(iNO3,k)
   Varout(oNO3,k) = (NO3+pp_ND+pp_NZ)/(1.+pp_PN/NO3)
   Varout(oZOO,k) = (ZOO+pp_ZP)/(1.+ EGES+ ZOO*dtdays*mz*tf_z + RES)
...
! Only calculate within the euphotic zone, otherwise many numerical problems.
      ...
      if( par_ .gt. I_zero ) then
       ...
       + (1.+RMchl/mu0hat)*larg1 * par_/zetaChl*daI_mu0hatdl
       ...
       dThetaHatdl = 1./par_*(-W_Y/larg/(1.+W_Y)*dlargdl*mu0hat/aI  &
     +  (1.-W_Y)*dmu0hat_aIdl)
       
       dWYYdl = dlargdl*(-W_Y**2/larg**2/(1.+W_Y)**3   &
       -  W_Y/larg**2/(1.+W_Y) + W_Y/(larg*(1.+W_Y))**2)
   
       d2ThetaHatdl2 = 1/par_*(-(W_Y/larg/(1.+W_Y)*dlargdl*dmu0hat_aIdl  &
        ...
        * ThetaHat + dThetaHatdl) * (1.-SI)*aI*par_/mu0hat    !confirmed
      ...
      * daI_mu0hatdl + (1.-SI)*aI/mu0hat*(                 &
      ...
      + (alphaI-dmu0hatdl/mu0hat)*dThetaHatdl + d2ThetaHatdl2)  )
       ...
       -zetaChl*(ThetaHat*d2mu0hatSIdl2+2.*dThetaHatdl*dmu0hatSIdl   &
...
! Calculate the mean chl:C ratio of phytoplankton (averaged over all sizes) 
    ! using the second derivative of the cell quota w.r.t. log size. (Why negative?)
       ...
       - ThetaHat*(d2fVdl2 + Qs*(dQNdl**2*(2./QN)-d2QNdl2)/(QN**2))), 0.01)
    ...
    ! using the second derivative of the cell quota w.r.t. log size. (How to derive?)
       QNAvg = max(QN/(1.+((2/QN)*dQNdl**2 - d2QNdl2)*VAR/(2.*QN)), 0.01)  
...
! self%dVARdt=VAR*VAR*(self%d2muNetdl2- self%d2gdl2bar-self%d2wdl2)  !Eq. 19 
   ...
   pp_PN=max(0.5*( PPpn + abs(PPpn)), 0.)
   pp_NP=max(0.5*(-PPpn + abs(PPpn)), 0.)
   
   DET1 = (DET + pp_DZ)/(1. + dtdays*rDN*tf_z)

   DET1 = min(max(DET1,1D-9),1D5)
   
   NO31 = (NO3+pp_ND+pp_NZ+pp_NP)/(1.+pp_PN/NO3)
    
   PHY1 =(PHY+pp_PN)/(1.+(pp_ZP+pp_NP )/PHY)
    
   ZOO1 = (ZOO+pp_ZP)/(1.+ EGES+ZOO*dtdays*mz*tf_z+RES)
