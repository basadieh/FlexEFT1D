PROGRAM main
  use BIO_rhs
  implicit none
  character(LEN=2 ), parameter :: Stn = 'S1'
  ! Input files
  ...
  ! Model options:
  integer,parameter  :: Model_ID       = EFTdiscrete
  ...
  real(8) :: Z_r(1:nlev), Z_w(0:nlev), Hz(nlev),  &  ! Grid variables
             ...
             Lsour(nlev),    Qsour(nlev), CHLA(nlev)             
...
4 format(I10,1x,I7, <nlev+1>(2x,F12.5))
  ...
  ! Calculate total Chl a:
     do k=1,nlev
        CHLA(k)=0.
        do i=1,NPHY
           cff   = Varout(iPHY(i),k)/Varout(oQN(i),k)*Varout(oTheta(i),k)
         CHLA(k) = CHLA(k) + cff
        enddo
     enddo 

     call Calculate_PAR(I_0, nlev, Hz, CHLA(:), PAR)
  ...
  do k = 1,nlev-1
     ...
     enddo
     ww(k,i+1)  = -Params(iwDET)             !Detritus      sinking rate
  enddo
  ...
  write(6,*) 'Model run finishes. CPU time:      ',t2-t1,' seconds'
