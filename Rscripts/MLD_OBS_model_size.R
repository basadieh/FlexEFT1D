...
pdf('ML_mod_obs_NChlNPP_EFTsize.pdf', width=5,height=6,paper='a4')
   op <- par(font.lab = 1,
                family ="serif",
                mar    = c(4,4,1,3),
                mgp    = c(2.3,1,0),
...
for (Var in c('TIN','CHL','NPP')){
  ...
  #Obtain MLD data:
    ...
    if (Var == 'NPP') Deu = abs(get_Deu(Stn,'EFTdiscrete'))
    for (i in 1:N){
       DOY_ = DOY_uni[i]
       MLD_ = MLD[DOY_]
    ...
    for (model in c('EFTdiscrete','EFTcont')){
       ...
       depth = Chl$depth
        ...
        if (Var == 'TIN' && Stn == 'S1') {
           legend('topright',c('Discrete','Continuous'),col=2:3,lty=1)
        }
    }
  }
}
...
pdf('ML_mod_obs_size_EFTsize.pdf', width=5,height=6,paper='a4')
   op <- par(font.lab = 1,
                family ="serif",
                mar    = c(4,4,1,3),
                mgp    = c(2.3,1,0),
...
for (Stn in c('S1','K2')){
  ...
  MLD=get_MLD(Stn)
  #Read observational data
  dat = paste('~/Working/FlexEFT1D/',Stn,'/',Stn,'_size.dat',sep='')
  dat = read.table(dat,header=T)
  ...
  for (Var in c('P10','P03','P01','P_1')){
    ...
    #Write out Ylab:
    if (Var == 'P10' && Stn =='S1'){
       YLab = expression(paste('Chl >10 µm (µg '*L^-1*')'))
       par(mar=c(2,4,2,.2))
    }else if(Var == 'P03' && Stn=='S1'){
       YLab = expression(paste('Chl 3~10 µm (µg '*L^-1*')'))
       par(mar=c(2,4,2,.2))
    } else if(Var == 'P01' && Stn=='S1'){
       YLab = expression(paste('Chl 1~3 µm (µg '*L^-1*')'))
       par(mar=c(2,4,2,.2))
    } else if (Stn=='S1'){
       YLab = expression(paste('Chl <1 µm (µg '*L^-1*')'))
       par(mar=c(4,4,2,.2))
    }
    ...
    if (Var == 'P_1' && Stn=='K2') par(mar=c(4,2,2,.2))
    #Plot data points:
    plot(DOY_uni,dat2,
      xlab=XLab,
      ylab=YLab,
